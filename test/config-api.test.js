...
test('public config exposes picker values and no OAuth secret', async () => {
  const handler = createConfigHandler({ GOOGLE_API_KEY: 'picker-key', GOOGLE_PROJECT_NUMBER: '123456', GOOGLE_CLIENT_ID: 'client.apps.googleusercontent.com', GOOGLE_CLIENT_SECRET: 'never-public' });
  const res = response();
...
test('public config returns a controlled error when picker config is incomplete', async () => {
  const res = response();
  await createConfigHandler({ GOOGLE_CLIENT_ID: 'id' })({ method: 'GET', headers: {} }, res);
