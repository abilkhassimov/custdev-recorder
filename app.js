import {
  ...
  moveBlock, moveQuestion, updateBlock, updateQuestion, safeFilename, buildMarkdown, migrateState, batchBlobs, canRecord
...
async function start() { await checkSession(); if (canRecord(session,settings)) renderStart(); else { step = session ? 1 : 0; renderOnboarding(); } }
...
function renderStart() {
  if(!canRecord(session,settings)){step=session?1:0;return renderOnboarding();}
...
async function beginRecording(){const project=document.querySelector('#project').value.trim(),interviewee=document.querySelector('#interviewee').value.trim();if(!project||!interviewee)return announce('Укажите проект и собеседника',true);if(!await checkSession()){step=0;renderAuth();return announce('Сессия Google истекла. Войдите снова.',true);}try{const stream=await navigator.mediaDevices.getUserMedia({audio:true});recording.stream=stream;recording.mime=pickMime();recording.chunks=[];recording.startedAt=new Date().toISOString();recording.project=project;recording.interviewee=interviewee;recording.block=0;recording.busy=false;recording.recorder=new MediaRecorder(stream,recording.mime?{mimeType:recording.mime,audioBitsPerSecond:48000}:{});recording.recorder.ondataavailable=e=>{if(e.data.size)recording.chunks.push(e.data);};recording.recorder.onstop=finishRecording;recording.recorder.onerror=()=>interruptRecording('Ошибка записи микрофона. Сохраните тишину вокруг и попробуйте снова.');stream.getTracks().forEach(track=>track.addEventListener('ended',()=>{if(recording.recorder?.state==='recording')interruptRecording('Микрофон отключён, запись была прервана.');},{once:true}));recording.recorder.start(15_000);window.addEventListener('beforeunload',guard);document.addEventListener('visibilitychange',visibilityWarning);renderLive();}catch(error){announce(error.name==='NotAllowedError'?'Разрешите доступ к микрофону':'Не удалось начать запись',true);}}
function visibilityWarning(){if(document.hidden&&recording.recorder?.state==='recording')announce('Не закрывайте и не выгружайте вкладку: запись может прерваться.',true);}
function interruptRecording(message){if(recording.busy)return;recording.busy=true;try{if(recording.recorder?.state!=='inactive')recording.recorder.stop();}catch{}recording.stream?.getTracks().forEach(x=>x.stop());clearInterval(recording.timer);announce(message,true);}
...
function stopRecording(){if(recording.busy)return;recording.busy=true;document.querySelectorAll('button').forEach(x=>x.disabled=true);recording.recorder.stop();recording.stream.getTracks().forEach(x=>x.stop());clearInterval(recording.timer);document.removeEventListener('visibilitychange',visibilityWarning);}
...
function renderSettings(){shell(`<section><p class="eyebrow">Настройки</p><h1>CustDev Recorder</h1><div class="settings-list"><div><b>Google Drive</b><small>${session?esc(session.email):'Подключение не проверено'}</small><div class="row">${button('reconnect','Переподключить','secondary')}${button('logout','Выйти','danger')}</div></div><div><b>Папка</b><small>${esc(settings?.folder?.name||'Не выбрана')}</small>${button('folder','Изменить папку','secondary')}</div><div><b>Опросник</b><small>${esc(settings?.questionnaire?.title||'Не добавлен')}</small><div class="row">${button('edit','Редактировать','secondary')}${button('replace','Заменить','secondary')}</div></div><div><b>Локальные настройки</b><div class="row">${button('export','Экспорт JSON','secondary')}${button('reset','Сбросить','danger')}</div></div></div><div class="notice">Локально хранятся только название и ID папки и ваш опросник. Google-токены находятся в защищённой серверной сессии, а не в localStorage.</div></section>`,button('close','Готово'));
  document.querySelector('#close').onclick=()=>canRecord(session,settings)?renderStart():(step=0,renderOnboarding());document.querySelector('#reconnect').onclick=()=>location.href='/api/auth/google';document.querySelector('#logout').onclick=async()=>{try{await api('/api/auth/logout',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});session=null;step=0;renderAuth();announce('Вы вышли из Google',false);}catch(e){announce(e.message,true);}};document.querySelector('#folder').onclick=()=>{step=1;renderOnboarding();};document.querySelector('#edit').onclick=()=>{draft=settings.questionnaire;renderEditor(false);};document.querySelector('#replace').onclick=()=>{step=2;renderOnboarding();};document.querySelector('#export').onclick=()=>{const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([JSON.stringify(settings,null,2)],{type:'application/json'}));a.download='custdev-recorder-settings.json';a.click();URL.revokeObjectURL(a.href);};document.querySelector('#reset').onclick=()=>{if(confirm('Удалить папку и опросник из этого браузера?')){localStorage.removeItem(STORAGE);settings=null;draft=null;step=session?1:0;renderOnboarding();}};
}
