...
export function createParseHandler({fetch:fetchImpl=fetch,env:provided}={}) { return async function parse(req,res) { try {
  assertMethod(req,'POST'); assertSameOrigin(req); const env=provided||getEnv(); requireSession(req,env.SESSION_SECRET); const type=String(req.headers['content-type']||'').toLowerCase(); let text;
